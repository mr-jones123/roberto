...
type LoginSubmitEvent = Parameters<NonNullable<JSX.IntrinsicElements["form"]["onSubmit"]>>[0]
...
export function LoginForm({ onLogin }: Props): JSX.Element {
  ...
  const handleSubmit = (e: LoginSubmitEvent) => {
    e.preventDefault()
    setError(null)
    setSubmitting(true)
    ...
    <div className="flex h-full flex-col items-center justify-center bg-[#1e293b] px-6">
      <div className="w-full max-w-xs">
        <div className="mb-6 text-center">
          ...
          <h2 className="text-sm font-semibold text-slate-200">{t("login.title")}</h2>
